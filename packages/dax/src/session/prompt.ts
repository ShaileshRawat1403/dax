  ...
  export const prompt = fn(PromptInput, async (input) => {
    ...
    void PM.append_event({
      ...
      payload: {
        ...
        provider_id: message.info.model.providerID,
        model_id: message.info.model.modelID,
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
  ...
  async function commandPM(input: CommandInput): Promise<MessageV2.WithParts> {
    const sub = input.arguments.trim().split(/\s+/)[0]?.toLowerCase() || "help"
    const rest = input.arguments.trim().slice(sub.length).trim()
    ...
    if (sub === "note") {
      ...
      return respondCommandText({
        input,
        text: [`Saved DSR note.`, `id: ${row.id}`, `day: ${row.day}`].join("\n"),
      })
    }
    ...
    if (sub === "rules") {
      if (rest.toLowerCase().startsWith("add ")) {
        const args = rest.slice(4).trim().split(/\s+/).filter(Boolean)
        if (args.length < 3) {
          return respondCommandText({
            input,
