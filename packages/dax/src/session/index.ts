import { Slug } from "@dax-ai/util/slug"
...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    if (!result.parentID && (Flag.DAX_AUTO_SHARE || cfg.share === "auto"))
      share(result.id)
        .then((share) => {
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".dax", "plans")
      ...
      return {
        cost: safe(
          new Decimal(0)
            ...
            // Note: reasoning tokens charged at same rate as output tokens (pending models.dev pricing update)
