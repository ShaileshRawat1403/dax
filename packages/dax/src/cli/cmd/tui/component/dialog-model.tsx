...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
    ...
    const allowed = new Set(
      [
        ...(showAll() ? sync.data.provider.map((x) => x.id) : Object.keys(CORE_PROVIDER_PRIORITY)),
        ...favorites.map((x) => x.providerID),
        ...recents.map((x) => x.providerID),
        current?.providerID,
        props.providerID,
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "dax" && model.id.includes("-nano"),
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
    ...
    const providerOptions = pipe(
      ...
      sortBy((provider) => CORE_PROVIDER_PRIORITY[provider.id] ?? 99, (provider) => provider.name),
      flatMap((provider) =>
        pipe(
          provider.models,
          entries(),
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "dax" && model.includes("-nano"),
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
    ...
    const popularProviders = !connected()
      ? pipe(
          providers().filter((option) => option.value in CORE_PROVIDER_PRIORITY),
          map((option) => {
            return {
              ...option,
