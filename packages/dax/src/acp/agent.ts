...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `dax auth login` in the terminal",
        name: "Login with dax",
        id: "dax-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Dax stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const daxProvider = providers.find((p) => p.id === "dax")
    if (daxProvider) {
      if (daxProvider.models["big-pickle"]) {
        return { providerID: "dax", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(daxProvider.models))
      if (best) {
        return {
