...
import { allWorkerReadiness, formatWorkerReadiness, probeBinaryVersion, workerReadiness } from "./worker-doctor"
...
describe("worker doctor readiness", () => {
  ...
  test("stored login is explicitly unverified when no environment key is required", async () => {
    const report = await workerReadiness({
    ...
    expect(report.ready).toBe(true)
    const auth = report.items.find((item) => item.label === "Auth")!
    expect(auth.value).toContain("stored login not checked")
  ...
  test("format is stable and omits credentials", async () => {
    ...
    expect(rendered).toContain("PREREQUISITES PRESENT")
...
test.skipIf(process.platform === "win32")(
  "version probe terminates a stalled executable",
  async () => {
    const dir = await mkdtemp(join(tmpdir(), "dax-doctor-probe-"))
    const binary = join(dir, "stalled")
    try {
      await writeFile(binary, "#!/bin/sh\nexec sleep 60\n", { mode: 0o700 })
      const started = Date.now()
      expect(await probeBinaryVersion("stalled", () => binary)).toBeUndefined()
