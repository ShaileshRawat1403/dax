...
export async function workerReadiness(input: WorkerDoctorInput): Promise<WorkerReadinessReport> {
    ...
    value:
      missingEnv.length === 0
        ? profile.requiredEnv && profile.requiredEnv.length > 0
          ? `${profile.requiredEnv.join(", ")} present; credential validity not checked`
          : "no required env; stored login not checked"
  ...
  items.push({
    ...
    value:
      injected && Object.keys(injected).length > 0
        ? `isolated per-run (${Object.entries(injected)
            .map(([k]) => k)
            .join(", ")})`
        : stateValue,
  })
  ...
  items.push({
    ...
    value: egressHosts.length > 0 ? `allowlist: ${egressHosts.join(", ")}` : "no egress hosts configured",
  })
...
export function formatWorkerReadiness(report: WorkerReadinessReport): string {
  ...
  lines.push(
    `Status${"".padEnd(width - 6 + 2)}${report.ready ? "PREREQUISITES PRESENT (authentication not verified)" : "NOT READY"}`,
  )
  return lines.join("\n")
}
