...
<html>
  <head>
    <title>Dax - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>Dax - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
export async function CodexAuthPlugin(input: PluginInput): Promise<Hooks> {
  return {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "dax"
      output.headers["User-Agent"] = `dax/${Installation.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`
