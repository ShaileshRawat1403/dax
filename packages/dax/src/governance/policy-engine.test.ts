...
describe("policy engine", () => {
  test("evaluate respects last matching wildcard rule", () => {
    ...
    expect(PolicyEngine.evaluate("shell", "package.json", rules).action).toBe("allow")
  ...
  test("disabled collapses edit-family tools behind edit permission", () => {
    ...
    const disabled = PolicyEngine.disabled(["edit", "write", "patch", "multiedit", "shell"], rules)
    ...
    expect(disabled.has("shell")).toBeFalse()
  })
})
