...
export namespace ProviderTransform {
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "dax" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      if (input.model.providerID.startsWith("dax")) {
        result["promptCacheKey"] = input.sessionID
  ...
  export function error(providerID: string, error: APICallError) {
    ...
    if (providerID === "google" && lower.includes("insufficient authentication scopes")) {
      return (
        ...
        "Use `GEMINI_API_KEY` (recommended) or reconnect with a supported Google OAuth configuration and include Gemini scopes " +
    ...
    if (
      ...
      return (
        ...
        "`GEMINI_API_KEY`, use a supported Google OAuth configuration, or run individual CLI subscription work through `dax worker run antigravity`."
      )
    }
