...
describe("Intent Interpreter", () => {
  ...
  describe("refineIntent", () => {
    ...
    it("should return a basic contract for general prompts", async () => {
      ...
      expect(result!.formattedPrompt).toContain("## Repo Impact")
      expect(result!.formattedPrompt).toContain("## Contract Delta")
      expect(result!.formattedPrompt).toContain("## Likely Targets")
      expect(result!.formattedPrompt).toContain("## Likely Writes")
      expect(result!.formattedPrompt).toContain("## Validation Plan")
      expect(result!.formattedPrompt).toContain("## Governance Hints")
      ...
      expect(result!.executionProfile?.writeScope).toBe("single_file")
      expect(result!.repoImpact?.targetFiles).toContain("src/main.ts")
      expect(result!.contractDelta?.inferredTargets?.[0]).toContain("src/main.ts")
      expect(result!.validationPlan?.postChange?.length).toBeGreaterThan(0)
      expect(result!.governanceHints?.likelyTriggers?.length).toBeGreaterThan(0)
    ...
    it("should include session-aware context and watchouts in fallback contracts", async () => {
      ...
      expect(result!.executionProfile?.approvalLikelihood).toBe("high")
