...
const managedConfigDir = process.env.DAX_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "dax.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "dax.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "dax.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "dax.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .dax directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const daxDir = path.join(dir, ".dax")
      await fs.mkdir(daxDir, { recursive: true })
      const agentDir = path.join(daxDir, "agent")
...
test("loads agents from .dax/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const daxDir = path.join(dir, ".dax")
      await fs.mkdir(daxDir, { recursive: true })

      const agentsDir = path.join(daxDir, "agents")
...
test("loads commands from .dax/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const daxDir = path.join(dir, ".dax")
      await fs.mkdir(daxDir, { recursive: true })

      const commandDir = path.join(daxDir, "command")
...
test("loads commands from .dax/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const daxDir = path.join(dir, ".dax")
      await fs.mkdir(daxDir, { recursive: true })

      const commandsDir = path.join(daxDir, "commands")
...
test("does not try to install dependencies in read-only DAX_CONFIG_DIR", async () => {
  ...
  const prev = process.env.DAX_CONFIG_DIR
  process.env.DAX_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.DAX_CONFIG_DIR
    else process.env.DAX_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable DAX_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "dax.json"),
        JSON.stringify({ $schema: "https://dax.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "dax.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .dax config
      const projectDir = path.join(dir, "project")
      const daxDir = path.join(projectDir, ".dax")
      await fs.mkdir(daxDir, { recursive: true })
      ...
      // Local .dax config with different plugins
      await Bun.write(
        path.join(daxDir, "dax.json"),
        JSON.stringify({
          $schema: "https://dax.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .dax config with some overlapping plugins
      await Bun.write(
        path.join(daxDir, "dax.json"),
        JSON.stringify({
          $schema: "https://dax.ai/config.json",
...
// Note: preload.ts sets DAX_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .dax config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .dax directory config enables it
      const daxDir = path.join(dir, ".dax")
      await fs.mkdir(daxDir, { recursive: true })
      await Bun.write(
        path.join(daxDir, "dax.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/dax")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/dax")
...
describe("getPluginName", () => {
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@dax/plugin@2.0.0")).toBe("@dax/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-dax@2.4.3", "file:///project/.dax/plugin/oh-my-dax.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.dax/plugin/oh-my-dax.js")
  })
  ...
  test("local plugin directory overrides global dax.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const daxDir = path.join(projectDir, ".dax")
        const pluginDir = path.join(daxDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("DAX_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["DAX_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["DAX_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["DAX_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .dax/ directories when flag is set", async () => {
    const originalEnv = process.env["DAX_DISABLE_PROJECT_CONFIG"]
    process.env["DAX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .dax directory with a command
          const daxDir = path.join(dir, ".dax", "command")
          await fs.mkdir(daxDir, { recursive: true })
          await Bun.write(path.join(daxDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .dax should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["DAX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DAX_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["DAX_CONFIG_DIR"]
      process.env["DAX_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["DAX_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["DAX_CONFIG_DIR"]
      } else {
        process.env["DAX_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("DAX_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["DAX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DAX_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["DAX_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["DAX_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from DAX_CONFIG_DIR, not project
