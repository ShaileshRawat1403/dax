...
This is historical phase-one evidence from 2026-08-20, preserved from the provider branch.
The current adapter supersedes its early invocation flags: explicit model selection,
`--mode accept-edits`, JSON terminal validation, and bounded process lifetime remain
authoritative. These probes are not release validation for the current binary.
