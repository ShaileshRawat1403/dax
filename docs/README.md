# DAX Documentation

This directory contains the canonical documentation for the DAX project. The documentation is organized into the following categories:

- [**Product**](./product/): Documents related to product behavior, distribution, release, and end-user guidance.
- [**Architecture**](./architecture/): Core architectural models and decision records that define the DAX runtime.
- [**Features**](./features/): Detailed specifications for individual DAX features and operators.

## Start Here

- [Product Start Here](./product/start-here.md)
- [Release Readiness](./product/release-readiness.md)
- [Distribution](./product/distribution.md)

## Non-Developers

- [Peer Pre-release Guide](./product/prerelease.md)
- [Audit Agent Guide](./product/audit-agent.md)
- [GitHub CI Integration](./product/integrations-github-ci.md)

## Shipped Product Docs

- [Providers](./product/providers.md)
- [Build on DAX](./product/build-on-dax.md)
- [Workflows](./product/WORKFLOWS.md)

## Contributor / Current Merge Target

- [Contributor Start Here](./product/contributor-start-here.md)
- [DAX Execution Model](./architecture/DAX_EXECUTION_MODEL.md)
- [DAX Workstation](./features/DAX_WORKSTATION.md)
- [Architecture Overview](./architecture/ARCHITECTURE.md)

## Architecture

- [ARCHITECTURE.md](./architecture/ARCHITECTURE.md)
- [DAX Skills Model](./architecture/DAX_SKILLS_MODEL.md)
- [DAX Trust Model](./architecture/DAX_TRUST_MODEL.md)

## Features

- [DAX Explore](./features/DAX_EXPLORE.md)
- [DAX Write Governance](./features/DAX_WRITE_GOVERNANCE.md)
- [DAX Plan](./features/DAX_PLAN.md)
- [DAX Workstation](./features/DAX_WORKSTATION.md)
